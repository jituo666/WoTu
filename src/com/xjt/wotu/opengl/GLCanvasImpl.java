...
import com.xjt.wotu.util.IntArray;
...
public class GLCanvasImpl implements GLCanvas {
    ...
    public void setAlpha(float alpha) {
        UtilsBase.assertTrue(alpha >= 0 && alpha <= 1);
        mAlpha = alpha;
    }
    ...
    @Override
    public void drawTexture(BasicTexture texture, float[] mTextureTransform, int x, int y, int w,
            int h) {
        mGLState.setBlendEnabled(mBlendEnabled
                && (!texture.isOpaque() || mAlpha < OPAQUE_ALPHA));
        if (!bindTexture(texture))
    ...
    public void drawMixed(BasicTexture from, int toColor, float ratio, int x, int y, int width, int height, float alpha) {
        ...
        mGLState.setBlendEnabled(mBlendEnabled && (!from.isOpaque()
                || !UtilsBase.isOpaque(toColor) || alpha < OPAQUE_ALPHA));
    ...
    private static class GLState {
        ...
        public void setColorMode(int color, float alpha) {
            setBlendEnabled(!UtilsBase.isOpaque(color) || alpha < OPAQUE_ALPHA);
