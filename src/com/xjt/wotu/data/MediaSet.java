...
import com.xjt.wotu.common.Future;
...
public abstract class MediaSet extends MediaObject {
    ...
    public MediaSet(MediaPath path, long version) {
        super(path, version);
    }
    ...
    public ImageItem getCoverMediaItem() {
        ArrayList<ImageItem> items = getMediaItem(0, 1);
        if (items.size() > 0) return items.get(0);
        for (int i = 0, n = getSubMediaSetCount(); i < n; i++) {
            ImageItem cover = getSubMediaSet(i).getCoverMediaItem();
    ...
    protected int getIndexOf(MediaPath path, ArrayList<ImageItem> list) {
    ...
    public static interface ItemConsumer {
        void consume(int index, ImageItem item);
    }
    ...
    protected int enumerateMediaItems(ItemConsumer consumer, int startIndex) {
        ...
        while (start < total) {
            ...
            ArrayList<ImageItem> items = getMediaItem(start, count);
